...
gender_x_purchase <- shopping |>
  select(Gender, NumberOfPurchases, PurchaseStatus, TimeSpentOnWebsite) |>
  group_by(Gender) |>
  summarise(
    Historical =sum(NumberOfPurchases, na.rm = TRUE),
    Purchases = sum(PurchaseStatus == 1, na.rm = TRUE),
    AverageTime = mean(TimeSpentOnWebsite, na.rm = TRUE),
    
    Total = n(),
    Purchase_Percent = Purchases / Total * 100) |>
  ungroup()
...
PurchaseStatus Percentage is split almost identically among males and females (slightly higher).

Question: Why is everything so evenly balanced?

### Group By Time Spent on Website

```{r}

time_spent <- shopping |> 
  group_by(TimeSpentOnWebsite) |>
  summarise()
...
Notes:
- It appears that the longer the mean time spent on a website, the greater the likelihood someone is to purchase something 
