...
# Loading data
work_dir <- here()
raw_data <- read_csv(file.path(work_dir, "shopping", "data", "raw_data", "uci_shopping.csv"), show_col_types =  FALSE)
head(raw_data)
```
...
for (i in 1:length(categorical_var)) {
  current_var_count <- shopping_uci %>% group_by(across(all_of(categorical_var[i]))) %>% summarize(count = n())
